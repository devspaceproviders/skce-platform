    ...
    >
      ...
      <div className="mb-5 flex items-center gap-4 text-xs text-slate-400">
        <span className="flex items-center gap-1">
          <Clock size={14} /> {course.durationMonths} Months
        </span>
        <span className="flex items-center gap-1">
      ...
      {/* View Details — animated link that reveals/slides its arrow on hover */}
      <Link
        href={`/courses/${course.slug}`}
        className="mb-4 inline-flex w-fit items-center gap-1.5 text-sm font-semibold text-brand transition-all duration-300 hover:gap-2.5"
      >
        View Details
        <ArrowRight
          size={15}
      ...
      <div className="flex items-center justify-between">
        ...
        {/* Enroll Now -> Contact page, with this course pre-selected in "Course of Interest" */}
        <Link
          href={`/contact?course=${course.slug}`}
          className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-700"
        >
          Enroll Now
        </Link>
      </div>
